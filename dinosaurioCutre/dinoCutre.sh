PRINCIPAL=$(mktemp -u "mainXXXXX.py")
...
# Obtener el PID del terminal actual
TERMINAL_PID=$$

# Pasar el PID como variable de entorno al comando bash -c
export TERMINAL_PID

# Ejecutar el script principal de Python con kitty y bash
kitty --override initial_window_width=50 --override initial_window_height=12 bash -c "python $PRINCIPAL; exec bash; kill \$TERMINAL_PID"

# Eliminar el archivo temporal después de que el juego haya terminado
rm $PRINCIPAL
