...
caracteres = list(str(input("Pon tus letras a combinar incluyendo repetidas: ")))
...
while len(arr) <= len(caracteres):
    isValido = True
    for num in arr:
        if arr.count(num) > 1: #Comprueba que los números no estén repetidos
            isValido = False
            break
    if isValido:
